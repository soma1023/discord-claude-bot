...
- **「音声も跳ねた候補だけ」の絞り込みでも、候補の質は変わらなかった。**

→ 音声解析は丸ごと取り外した。ffmpeg の依存も消え、1本あたり数分の
  ダウンロードが不要になった。実装はコミット履歴に残っている
  （`be750e4` 以前）ので、必要になれば戻せる。

## 次にやること

1. **実際の切り抜き作業で使ってみる。** 機能としては一通り揃った。
2. **チャット側の候補の質を上げる。** 改善の余地があるとすればここ。
